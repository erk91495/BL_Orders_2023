        ...
        container.Column(column =>
        {
            ...
            column.Item().Table(table =>
            {
                ...
                table.Header(header =>
                {
                    ...
                    header.Cell().Element(CellStyle).Text("lbs.").Style(tableHeaderStyle);
                    header.Cell().Element(CellStyle).Text("$").Style(tableHeaderStyle);
                    header.Cell().Element(CellStyle).Text("% of Total $ Sales").Style(tableHeaderStyle);
                    header.Cell().Element(CellStyle).Text("% of Total lbs. Sales").Style(tableHeaderStyle);
                ...
                var groupedItems = _items.GroupBy(i => i.Product.Category).OrderBy(i => i.Key.DisplayIndex);
                ...
                foreach (var category in groupedItems)
                {
                    ...
                    table.Cell().Element(CellStyle).Text($"{category.Sum(i => i.QuantityReceived)}").Style(tableTextStyle);
                    var totalWeight = category.Sum(i => i.PickWeight);
                    table.Cell().Element(CellStyle).Text($"{totalWeight:N2}").Style(tableTextStyle);
                    var totalPrice = category.Sum(i => i.GetTotalPrice);
                    table.Cell().Element(CellStyle).Text($"{totalPrice:C}").Style(tableTextStyle);
                    table.Cell().Element(CellStyle).Text($"{(totalPrice / totalDollars) * 100m:N2}").Style(tableTextStyle);
                    table.Cell().Element(CellStyle).Text($"{(totalWeight / totalPounds) * 100f:N2}").Style(tableTextStyle);
                    static IContainer CellStyle(IContainer container)
                    {
